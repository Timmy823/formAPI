...
class AccessSheetData {
        ...
        if(filetype === "xlsx" || filetype === "lsx" || filetype === "csv"){
                ...
                workbook.eachSheet((worksheet, sheetId)=>{
                    ...
                    worksheet.eachRow({ includeEmpty: false }, (row, rowNumber)=>{
                        if(rowNumber == 1){
                            ...
                            queryInfo = "insert into " + this.schema + "." + worksheet.name;
                            queryInfo += " values ("
                            for(let i = 1; i < row.values.length; i++){
            ...
            fs.readFile(path, "utf8", (err, content) => {
                ...
                Object.keys(fileContent).map((table) => {
                    let queryInfo = "create table if not exists " + this.schema + "." + table;
                    newSheet.push(table);
                    queryInfo += "(";
    ...
    listdata(sheetName, callback){
        ...
        */
        ...
        .then(([results, fields])=>{
            ...
            if(len_ == 0){
                queryInfo = "select column_name from information_schema.columns where table_name = '" + sheetName + "'";
                this.connection.query(queryInfo, (err, results, fields) => {
    ...
    insertdata(sheetName, inputData, callback){
        ...
        .then(()=>{
            ...
            queryInfo = "select count(*) as totalRow from " + this.schema + "." + sheetName;
            this.connection.query(queryInfo, (err, results,fields)=>{
